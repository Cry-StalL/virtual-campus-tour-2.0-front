...
const configFiles = import.meta.glob('/src/assets/data/street-viewer-config*.json', { eager: true, import: 'default' });

function getNewestConfigData(prefix: string): { data: any, fileName: string } {
  // 匹配 street-viewer-config.json 和 street-viewer-config-时间戳.json
...
export function useStreetViewerConfig() {
  ...
  onMounted(async () => {
    try {
      ...
      let data, fileName;
      if (mode === 'fixed_file') {
        data = configFiles['/src/assets/data/street-viewer-config.json'];
        fileName = 'street-viewer-config.json';
        if (!data) throw new Error('配置文件加载失败');
      } else {
        const result = getNewestConfigData('street-viewer-config');
        data = result.data;
